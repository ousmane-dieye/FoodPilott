  ...
  return (
    <Routes>
      ...
      <Route
        path="notifications"
        element={
          <PlaceholderView title="Alertes Live" icon={<Bell size={40} />} />
        }
      />
      <Route
        path="profile"
      ...
      <Route
        path="menu"
        element={
          <PlaceholderView title="Menu du Jour" icon={<Utensils size={40} />} />
        }
      />
  ...
  return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-8 bg-slate-900/5 dark:bg-slate-900/40 rounded-[48px] border-2 border-dashed border-slate-200 dark:border-slate-800">
      ...
      <div className="text-center space-y-2">
        <h2 className="text-4xl font-black tracking-tightest text-slate-900 dark:text-white italic">
          {title}
        </h2>
        <p className="text-slate-500 font-medium">
          Station de production FoodPilot — Module en attente d'activation.
        </p>
      </div>
      <Link to="/kitchen/ops">
        <Button
          variant="secondary"
          className="px-8 h-14 rounded-2xl font-black italic bg-slate-900 text-white hover:bg-slate-800"
        >
          RETOUR AUX OPS
        </Button>
      </Link>
  ...
  const simulateScan = () => {
    ...
    setTimeout(() => {
      const readyOrder = orders.find(
        (o) => o.status === "ready" && o.paymentStatus === "paid",
      );
      if (readyOrder) {
    ...
    <div className="space-y-12">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          <Card
            key={i}
            className="p-6 border-2 border-slate-100 dark:border-slate-800"
          >
        ...
        <div className="xl:col-span-3 space-y-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h2 className="text-4xl font-black tracking-tightest text-slate-900 dark:text-white italic uppercase">
                  Kitchen Ops
                </h2>
                <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">
                  Station #01 • Production Live
                </p>
              </div>
            </div>

            <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl ring-2 ring-slate-200">
              {(["pending", "preparing", "ready", "served"] as const).map(
                (tab) => {
                    ...
                    <div className="p-8">
                      <div className="flex justify-between items-start mb-8 pb-6 border-b border-slate-100">
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <h4 className="text-3xl font-black tracking-tightest text-slate-900 italic">
                              {order.id}
                            </h4>
                            <Badge
                              className={cn(
                        ...
                        <div className="text-right">
                          <Badge
                            variant="outline"
                            className="font-mono text-xs font-black border-2 border-slate-200 bg-slate-50 text-slate-900 px-3 py-1 mb-2 block"
                          >
                            {order.slot}
                          </Badge>
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-tightest">
                            PROCHE SORTIE
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 mb-10">
                        {order.items.map((item: any, i: number) => (
                          <div
                            key={i}
                            className="flex items-center justify-between p-5 bg-slate-50 rounded-2xl ring-1 ring-slate-100 shadow-sm border-l-4 border-primary"
                          >
                      ...
                      <div className="flex gap-3">
                        {order.status === "pending" && (
                          <Button
                            ...
                            className="flex-1 h-14 bg-orange-500 hover:bg-orange-600 font-black text-white rounded-xl shadow-soft uppercase text-xs tracking-widest border-none"
                          >
                            Lancer Cuisine
                          </Button>
                        )}
                        {order.status === "preparing" && (
                          <Button
                            ...
                            className="flex-1 h-14 bg-blue-500 hover:bg-blue-600 font-black text-white rounded-xl shadow-soft uppercase text-xs tracking-widest border-none"
                          >
                            Terminer
                          </Button>
                        )}
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-14 w-14 bg-slate-100 rounded-xl text-slate-600 hover:bg-red-50 hover:text-red-600"
                        >
                          <AlertCircle size={22} />
                        </Button>
                      </div>
          ...
          <Card className="h-full flex flex-col p-8 sticky top-24 border-2 border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900">
            ...
            <div className="flex-1 space-y-5 mb-10 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
              <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-lg border-l-4 border-primary">
                ...
                <p className="text-sm font-medium leading-relaxed">
                  Pic d'affluence prévu dans 15 min. Attention au stock Thieb.
                </p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl ring-2 ring-slate-100 dark:ring-slate-700 shadow-sm">
                ...
                <p className="text-sm font-bold text-slate-900 dark:text-white leading-relaxed italic">
                  Table 4 demande du sel et de l'eau.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-5 mt-auto pt-8 border-t-2 border-slate-100 dark:border-slate-800">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  size="icon"
                  className="h-14 w-14 min-w-[56px] rounded-xl bg-slate-900 text-white shadow-lg"
                >
                  <Send size={20} />
                </Button>
              </div>
            ...
            >
              {!scannedOrder ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-500 text-xl font-bold uppercase tracking-widest max-w-xs mx-auto">
                    Veuillez présenter le QR Code de l'étudiant.
                  </p>
                  <Button
                    variant="ghost"
                    onClick={() => setScanning(false)}
                    className="mt-12 h-14 px-10 text-slate-400 hover:text-red-500 font-black text-xs uppercase tracking-widest"
                  >
                    Annuler l'opération
                  </Button>
                </div>
                ...
                <div className="space-y-10">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-4">
                        <Badge className="bg-emerald-500 text-white border-none py-2 px-6 font-black tracking-widest">
                          ORDRE DÉFECTÉ
                        </Badge>
                        <span className="text-xs font-black uppercase text-slate-400 tracking-tightest">
                          {new Date().toLocaleTimeString()}
                        </span>
                      </div>
                      <h3 className="text-6xl font-black tracking-tightest mt-4 italic text-slate-900 dark:text-white uppercase">
                        {scannedOrder.id}
                      </h3>
                    </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800 rounded-[40px] p-10 ring-2 ring-slate-100 dark:ring-slate-700 shadow-inner">
                    <p className="text-[11px] font-black uppercase text-slate-400 tracking-widest mb-6 pb-4 border-b border-slate-200 dark:border-slate-700">
                      DÉTAILS COMMANDE HP-01
                    </p>
                    <div className="space-y-4">
                      {scannedOrder.items.map((item: any, i: number) => (
                        <div
                          key={i}
                          className="flex justify-between items-center text-2xl font-black text-slate-900 dark:text-white italic"
                        >
                          <span>
                            {item.quantity}x {item.name}
                          </span>
          ...
          <div className="relative flex flex-col md:flex-row justify-between items-center gap-10">
            <div className="flex items-center gap-8">
              ...
              <div className="max-w-md">
                <h4 className="text-3xl font-black tracking-tightest mb-2 italic uppercase">
                  Stocks Critiques
                </h4>
                <p className="text-slate-400 text-lg font-medium leading-relaxed">
                  Attention Chef ! Le stock de{" "}
                  <span className="text-white font-black underline">
                    "Pain Burger"
                  </span>{" "}
                  est à moins de 15%. Réapprovisionnement urgent requis.
                </p>
              </div>
            </div>
            <div className="flex gap-4 w-full md:w-auto">
              <Button
                variant="secondary"
                className="bg-white/10 text-white border-none hover:bg-white/20 px-10 h-16 rounded-2xl flex-1 md:flex-none font-bold uppercase text-xs tracking-widest"
              >
                Ignorer
              </Button>
              <Button className="bg-white text-slate-900 hover:bg-slate-200 px-10 h-16 rounded-2xl flex-1 md:flex-none font-black uppercase text-xs tracking-widest shadow-xl">
                Gérer Stocks
              </Button>
            </div>
          </div>
