...
export default function AffluenceView() {
  ...
  const getAffluenceState = () => {
    if (activeOrders > 8 || occupiedTables > 25)
      return {
        label: "Plein",
        color: "bg-red-500",
        text: "affluence_critical",
        desc: "Restaurant très fréquenté - temps d’attente élevé",
      };
    if (activeOrders > 3 || occupiedTables > 12)
      return {
        label: "Moyen",
        color: "bg-amber-500",
        text: "affluence_medium",
        desc: "Fréquentation modérée - service fluide",
      };
    return {
      label: "Vide",
      color: "bg-emerald-500",
      text: "affluence_low",
      desc: "Restaurant peu fréquenté - service ultra rapide",
    };
  };
  ...
  return (
    <div className="space-y-12">
      <header>
        <h2 className="text-5xl font-black tracking-tightest text-ink italic mb-2 uppercase">
          Affluence Live
        </h2>
        <p className="text-ink-secondary font-black uppercase tracking-widest text-base">
          Temps réel & Prévisions • Restaurant ESMT
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <Card className="p-12 flex flex-col items-center justify-center text-center overflow-hidden relative group">
          ...
          >
            ...
            <h3 className="text-6xl font-black tracking-tightest text-ink mb-2 uppercase">
              {state.label}
            </h3>
            <p
              className={cn(
            ...
            <Badge
              variant="secondary"
              className="px-8 py-3 rounded-2xl bg-surface text-ink font-bold text-lg shadow-soft border-2 border-gray-100"
            >
              ~
              {state.label === "Vide"
                ? "5"
        ...
        <div className="grid grid-cols-2 gap-6">
          <Card className="p-8 flex flex-col justify-between hoverable border-2 border-gray-100 bg-white">
            ...
            <div>
              <p className="text-5xl font-black text-ink mb-1">
                {activeOrders}
              </p>
              <p className="text-[11px] font-black text-ink-secondary uppercase tracking-widest leading-relaxed">
          ...
          <Card className="col-span-2 p-8 bg-ink text-white shadow-large relative overflow-hidden group">
            ...
            <div className="relative">
              <Badge className="bg-white/20 text-white mb-4 border-none px-4 py-1.5 font-bold">
                INFO ESMT
              </Badge>
              <h4 className="text-2xl font-black tracking-tight mb-2 italic">
                Prochain pic prévu à 12:30
              </h4>
              <p className="text-gray-400 font-medium">
                Réservez dès maintenant pour éviter la file d'attente de la
                pause méridienne.
              </p>
            </div>
          </Card>
      ...
      <section className="space-y-8">
        <header className="flex justify-between items-end">
          <div>
            <h3 className="text-2xl font-black tracking-tight text-ink uppercase italic mb-1">
              Évolution de la Journée
            </h3>
            <p className="text-sm font-bold text-ink-secondary uppercase tracking-widest">
              Moyenne basée sur les 30 derniers jours
            </p>
          </div>
      ...
      <Card className="p-8 border-dashed border-2 flex items-center gap-6 bg-surface/50">
        ...
        <div className="flex-1">
          <h4 className="text-xl font-black text-ink italic mb-1">
            Conseil du Chef
          </h4>
          <p className="text-ink-secondary font-medium italic">
            "Pour un service en moins de 10 minutes, nous vous conseillons de
            passer commande avant 11:45 ou après 13:15."
          </p>
        </div>
        <Link to="/student/order">
          <Button className="rounded-[20px] font-black italic px-8">
