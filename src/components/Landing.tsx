...
export default function Landing() {
  ...
  const [authView, setAuthView] = useState<
    "login" | "register-student" | "register-client" | "forgot"
  >("login");
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authView === "login") {
        const profile = await AuthService.loginWithEmail(
          loginData.email,
          loginData.password,
        );
        if (profile) {
  ...
  const demoCards = [
    {
      ...
      title: "Étudiant",
      desc: "Commander, Fidélité, IA Nutrition",
      icon: User,
    ...
    },
    {
      ...
      title: "Cuisinier",
      desc: "Gestion Kitchen, Commandes, IA Recettes",
      icon: ChefHat,
    ...
    },
    {
      ...
      title: "Admin",
      desc: "Analytics, Stocks, Pilotage Global",
      icon: BarChart3,
      color: "text-blue-500",
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background">
        <Card className="max-w-md w-full text-center p-12">
          ...
          <h2 className="text-3xl font-black tracking-tighter text-ink mb-2">
            Bienvenue
          </h2>
          <p className="text-gray-400 font-medium mb-8">
            Voulez-vous retourner à votre interface de pilotage ?
          </p>
          <div className="space-y-3">
            <Button
              onClick={() =>
                redirectByRole(
                  useAuthStore.getState().profile?.role || "STUDENT",
                )
              }
              className="w-full"
            >
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col items-center justify-center py-20 px-6">
      ...
      <div
        className="absolute bottom-0 -right-20 w-96 h-96 bg-blue-500/10 rounded-full blur-[100px] animate-pulse"
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center z-10">
        ...
        >
          ...
          <p className="text-2xl text-ink-secondary font-bold max-w-lg leading-relaxed opacity-100 italic">
            FoodPilot redéfinit l'expérience culinaire universitaire à l'ESMT.
            Une plateforme intelligente pour les étudiants, les cuisiniers et
            les gestionnaires.
          </p>
          ...
          <div className="flex items-center gap-6 pt-6">
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-10 h-10 rounded-full border-4 border-background bg-gray-200 overflow-hidden ring-1 ring-primary/20"
                >
            ...
            <p className="text-sm font-bold text-ink">
              <span className="text-primary">+500 étudiants</span> déjà
              connectés au pilotage.
            </p>
          </div>
        </motion.div>
          ...
          <Card className="p-10 md:p-12 border-none shadow-large bg-surface/80 backdrop-blur-xl relative overflow-hidden group">
            ...
            <div className="relative">
              <div className="mb-12">
                ...
                <p className="text-base text-ink-secondary font-medium mt-2">
                  Connectez-vous à la Zone FoodPilot Network
                </p>
              </div>
                ...
                >
                  {authView === "login" || authView === "forgot" ? (
                    ...
                    <div className="space-y-4 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                      ...
                      <div className="space-y-3">
                        <label className="text-sm font-bold text-ink-secondary uppercase tracking-wider">
                          Allergies
                        </label>
                        <div className="grid grid-cols-2 gap-3">
                          {allergyOptions.map((a) => (
                            <label
                              key={a}
                              className="flex items-center gap-2 p-4 bg-background rounded-2xl cursor-pointer hover:border-primary border border-transparent transition-all shadow-sm"
                            >
                              <input
                                type="checkbox"
                                checked={studentData.allergies.includes(a)}
                                onChange={(e) => {
                                  const news = e.target.checked
                                    ? [...studentData.allergies, a]
                                    : studentData.allergies.filter(
                                        (x) => x !== a,
                                      );
                                  setStudentData({
                                    ...studentData,
                              ...
                              />
                              <span className="text-sm font-bold text-ink-secondary capitalize">
                                {a}
                              </span>
                            </label>
                          ))}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {authView !== "register-student" && (
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => setAuthView("register-student")}
                  ...
                  {authView !== "register-client" && (
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => setAuthView("register-client")}
