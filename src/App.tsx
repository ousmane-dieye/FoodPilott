...
const roleModules = {
  ADMIN: [
    ...
    { id: 'reservations', label: 'Flux', icon: ShieldCheck, path: '/admin/reservations' },
    { id: 'orders', label: 'Ventes', icon: ShoppingBag, path: '/admin/orders' },
    { id: 'kitchen', label: 'Cuisine', icon: ChefHat, path: '/admin/kitchen' },
    { id: 'stock', label: 'Gestion Stocks', icon: Package, path: '/admin/stock' },
    { id: 'ai', label: 'Pilot IA', icon: Sparkles, path: '/admin/ai' },
    { id: 'affluence', label: 'Affluence', icon: LayoutGrid, path: '/student/affluence' },
    { id: 'antigaspillage', label: 'Anti-Gaspi', icon: Leaf, path: '/admin/antigaspi' },
  ...
  COOK: [
    ...
    { id: 'stock', label: 'Alertes Stocks', icon: Package, path: '/kitchen/stock' },
    { id: 'notifications', label: 'Alertes Live', icon: Bell, path: '/kitchen/notifications' },
    { id: 'auth', label: 'Mon Profil', icon: User, path: '/kitchen/profile' },
  ],
  STUDENT: [
    { id: 'menu', label: 'Passer Commande', icon: ShoppingBag, path: '/student/order' },
    { id: 'affluence', label: 'Affluence', icon: LayoutGrid, path: '/student/affluence' },
    { id: 'history', label: 'Historique', icon: Clock, path: '/student/history' },
    { id: 'fidelity', label: 'Points Cadeaux', icon: Trophy, path: '/student/fidelity' },
        ...
        <header className="fixed top-0 left-0 right-0 z-50 glass h-20 px-6 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block">
              <h1 className="text-xl font-bold tracking-tight text-ink leading-none">FoodPilot</h1>
              <p className="text-sm font-bold text-primary uppercase tracking-wider mt-0.5">ESMT Innovate</p>
            </div>
          </Link>

          <div className="flex items-center gap-2 sm:gap-4">
            {isDemoMode && <Badge variant="warning" className="hidden md:flex animate-pulse">Mode Démo</Badge>}

            {user ? (
              <div className="flex items-center gap-4 pl-4 border-l border-gray-200">
                <div className="hidden sm:flex flex-col items-end">
                  <p className="text-sm font-bold text-ink uppercase tracking-tight">{profile?.displayName || user.displayName}</p>
                  <p className="text-xs font-bold text-ink-secondary uppercase tracking-tight">{profile?.role || 'STUDENT'}</p>
                </div>
                ...
                <Button variant="ghost" size="icon" onClick={() => AuthService.logout()} className="text-gray-400 hover:text-red-500">
                  <LogOut size={20} />
                </Button>
              </div>
            ) : (
  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 h-[calc(100vh-80px)] sticky top-20 border-r border-gray-100 dark:border-slate-800 p-6 overflow-y-auto custom-scrollbar">
      <div className="space-y-3">
        <p className="text-sm font-bold text-ink-secondary uppercase tracking-widest mb-6 ml-4">Pilotage</p>
        <Link 
            to="/" 
            className={cn(
              "flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all font-bold text-base",
              location.pathname === '/' ? "bg-gray-100 text-ink" : "text-ink-secondary hover:text-ink hover:bg-gray-50 dark:hover:bg-slate-800/50"
            )}
          >
            <Home size={22} />
            <span>Accueil</span>
...
function MobileNav({ role }: { role: UserRole }) {
  ...
  return (
    <nav className="lg:hidden fixed bottom-6 left-6 right-6 z-50 glass shadow-large rounded-[32px] px-4 py-3 flex items-center justify-around">
      <Link to="/" className={cn("p-3 rounded-2xl transition-all", location.pathname === '/' ? "bg-primary text-white scale-110 shadow-soft" : "text-ink-secondary")}>
        <Home size={24} />
      </Link>
      {modules.map((m) => {
        const isActive = location.pathname.startsWith(m.path);
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              isActive ? "bg-primary text-white scale-110 shadow-soft" : "text-ink-secondary hover:text-ink"
            )}
          >
            <Icon size={24} />
          </Link>
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-ink mb-4 tracking-tight">Accès Défié</h2>
          <p className="text-ink-secondary font-medium text-base leading-relaxed mb-8">
            Le code d'accès de votre rôle actuel ({currentRole || 'NON IDENTIFIÉ'}) n'est pas autorisé ici.
          </p>
          <Link to="/">
            <Button variant="primary" className="w-full">Vers la Zone Sûre</Button>
          </Link>
        </motion.div>
      </div>
    );
      ...
      <div className="h-screen flex items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold tracking-tight text-ink">FoodPilot</p>
            <p className="text-sm font-bold text-primary uppercase tracking-[0.3em] animate-pulse">Initialisation...</p>
          </div>
        </div>
      </div>
    );
