...
export const MockOrderService = {
        ...
        const defaults: MockOrder[] = [
            {
                id: '#4581',
                userId: 'demo_user',
                items: [{ id: '1', name: 'Thieboudienne Rouge', price: 2500, quantity: 1 }],
                total: 2500,
                status: 'completed',
                paymentMethod: 'wave',
                paymentStatus: 'paid',
            ...
            },
            {
                id: '#4582',
                userId: 'demo_user',
                items: [{ id: '2', name: 'Burger Pilot Triple', price: 3500, quantity: 1 }],
                total: 3500,
                status: 'served',
                paymentMethod: 'cash',
                paymentStatus: 'paid',
  ...
  createOrder(orderData: Omit<MockOrder, 'id' | 'createdAt' | 'status' | 'qrData'>): MockOrder {
  ...
  updateOrderStatus(orderId: string, status: MockOrder['status'] | MockOrder['paymentStatus']) {
    const orders = this.getOrders();
    const index = orders.findIndex(o => o.id === orderId);
    if (index !== -1) {
      const orderStatusList: MockOrder['status'][] = ['pending', 'preparing', 'ready', 'served', 'completed', 'cancelled'];
      const paymentStatusList: MockOrder['paymentStatus'][] = ['paid', 'failed', 'cash_pending'];
  ...
  getReviews(): MockReview[] {
    ...
    if (!data) {
      ...
      const defaults: MockReview[] = [
        { id: '1', itemId: 1, userName: 'Aminata B.', rating: 5, comment: 'Le meilleur Thieb de Dakar !', createdAt: '18/04/2026 12:00' },
        { id: '2', itemId: 1, userName: 'Moussa F.', rating: 4, comment: 'Très copieux, un peu épicé.', createdAt: '18/04/2026 11:30' },
        { id: '3', itemId: 2, userName: 'Ousseynou D.', rating: 5, comment: 'Le burger est énorme, je recommande.', createdAt: '18/04/2026 10:45' },
      ];
